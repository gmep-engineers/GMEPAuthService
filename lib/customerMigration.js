...
const processPhone = async function (conn, itemPhone, entityId) {
  ...
  query = `
      insert into phone_number_entity_rel (id, phone_number_id, entity_id, is_primary) values (?, ?, ?, 1)
      `;
...
const createNewCustomer = async function (conn, item) {
...
const updateExistingCustomer = async function (conn, item) {};

const processCustomer = async function (conn, item) {
  ...
  if (update) {
    ...
    await updateExistingCustomer(conn, item);
  } else {
    console.log("creating new customer");
    await createNewCustomer(conn, item);
  }
};
...
const processSingleCustomer = async function (conn, customerId, token) {
  var offset = 0;
  headers = {
