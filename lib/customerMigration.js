...
const createContactFromIndividualCustomer = async function (conn, item) {
  const contactId = `i${item.id}`;
  var query = `
    select entity_id from contacts where id = ?
    `;
  var [result] = await conn.query(query, [contactId]);
  if (result.length > 0) {
    // await updateExistingContact(conn, item, result[0].entity_id);
    return;
  }

  const contactEntityId = uuidv1();
  var query = `
      insert into entities (id) values (?)
    `;
  await conn.query(query, [contactEntityId]);
  query = `
  ...
  try {
    ...
    if (item.email) {
      await processEmail(conn, item.email, contactEntityId);
    }

    if (item.phone) {
      await processPhone(conn, item.phone, contactEntityId);
    }

    if (item.mobilephone) {
      await processPhone(conn, item.mobile, contactEntityId);
    }
  } catch (err) {
    console.error(err);
  }
...
const createNewCustomer = async function (conn, item, isArchitect) {
  ...
  if (item.isperson && item.isperson === "T") {
    await createContactFromIndividualCustomer(conn, item);
    return;
  }
